...
class Opt:
    def __init__(self, imauto=1, imblev=8, impalred=15, rgsim=5, rgsize=1.5, blsim=200, blcoldev=0.2, blint=0.2):
        # 这些值将作为默认值使用
        self.imauto = imauto         # 自动搜索相似区域
        self.imblev = imblev         # 图像模糊程度
        self.impalred = impalred      # 图像调色板减少因子
        self.rgsim = rgsim          # 区域相似性阈值
        self.rgsize = rgsize      # 区域大小阈值
        self.blsim = blsim        # 块相似性阈值
        self.blcoldev = blcoldev    # 块颜色偏差阈值
        self.blint = blint        # 块交集阈值
