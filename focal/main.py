...
class FOCAL(nn.Module):
    ...
    def process(self, Ii, Mg, isTrain=False, threshold=0):
        ...
        if isTrain:
            ...
            distances = torch.cdist(Fo, centroids)
            max_rate = 0
            for idx in range(B):
                Lo = Lo_batch[idx]
...
def detect_folder(input_path, output_path, gpu):
    ...
    for file in os.listdir(input_path):
        # 读取图片，得到原始尺寸
        forgery_img = cv2.imread(os.path.join(input_path, file))
        H, W, _ = forgery_img.shape

        # 模型输入预处理：将被修改图像resize到指定尺寸，并归一化处理
        resized_img = cv2.resize(forgery_img, (input_size, input_size))
        # 注意这里将 BGR 转换为 RGB（通过[..., ::-1]），确保和模型预期一致
        img_tensor = torch.from_numpy(np.ascontiguousarray(resized_img[..., ::-1].transpose(2, 0, 1))).float() / 255.
        img_tensor = img_tensor.unsqueeze(0).cuda()  # 增加 batch 维度

        # 模型预测（得到低分辨率的检测结果），注意此处 isTrain=False
        for threshold in [50, 60, 65]:
            with torch.no_grad():
                detection, rate = model.process(img_tensor, None, isTrain=False, threshold=threshold * 0.01)
                if threshold == 50:
                    rates.append(rate)
            # 将检测结果转换成 numpy 数组，并resize回原图尺寸
            detection_np = convert(detection)[0]
            detection_resized = cv2.resize(detection_np, (W, H))
            detection_mask = thresholding(detection_resized)        
            cv2.imwrite(os.path.join(output_path, 'focal_threshold=' + str(threshold) + '_' + file), detection_mask.astype(np.uint8))

    rates = np.array(rates)
    np.save(os.path.join(output_path, 'rates.npy'), rates)
...
if __name__ == '__main__':
    ...
    detect_folder(args.input_path, args.output_path, args.gpu_index)
